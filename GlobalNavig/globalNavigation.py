...
def generateGraph(image, start, start_node, target, target_node, offset, threshold):
    ...
    large_polygons = find_enlarged_polygons(enlarged_img, threshold)
    ...
    connections = find_connections(enlarged_img, nodes_list, maxx=image.shape[1], maxy=image.shape[0])
...
def run_global(image, start_node, start, target_node, target, offset, threshold):
    start = [int(start[0]), int(start[1])] # convert to int for binary map check later
